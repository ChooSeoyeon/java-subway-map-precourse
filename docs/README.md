...
- [ ] 노선(PK, UK)과 역 목록으로 등록을 생성한다 (종점역을 노선에 등록)
    - [ ] 등록할 역이 두 개 미만일 경우 예외를 발생시킨다
- [ ] 역 목록에 역을 추가한다
    - [ ] 이미 목록에 존재하는 역을 추가할 경우 예외를 발생시킨다
- [ ] 역 목록에서 역을 삭제한다
    - [ ] 목록에 존재하지 않는 역을 삭제할 경우 예외를 발생시킨다
    - [ ] (삭제 전) 역 목록에 역이 세 개 미만일 경우 예외를 발생시킨다
...
- [ ] 빈 역 목록으로 저장소를 생성한다
- [ ] 역을 저장한다 (역 목록에 역을 추가) (역 목록 예시 : 강남역, 신림역, 양재역, 보라매역)
    - [ ] 중복된 역 이름인 경우 예외를 발생시킨다
- [ ] 역 이름으로 역을 삭제한다 (역 목록에서 역을 제거)
- [ ] 역 이름으로 역이 존재하는지 확인한다
- [ ] 역 목록을 조회한다

### LineRepository (노선 저장소)

- [ ] 빈 노션 목록으로 저장소를 생성한다
- [ ] 노선을 저장한다 (노선 목록에 노선을 추가) (노선 목록 예시 : 2호선, 신분당선)
    - [ ] 중복된 노선 이름인 경우 예외를 발생시킨다
- [ ] 노선 이름으로 노선을 삭제한다 (노선 목록에서 노선을 제거)
- [ ] 노선 이름으로 노선을 조회한다
- [ ] 노선 이름으로 노선이 존재하는지 확인한다
- [ ] 노선 목록을 조회한다

### RegistrationRepository (등록 정보 저장소 == 노선의 구간 목록 저장소)

- [ ] 빈 등록 정보 목록으로 저장소를 생성한다
- [ ] 등록 정보를 저장한다 (등록 정보 목록 예시 : { 2호선, [강남역, 신림역] } , { 신분당선, [강남역, 양재역] })행 -> 왼쪽 끝이 상행 종점
- [ ] 노선으로 등록 정보가 존재하는지 확인한다
- [ ] 노선으로 등록 정보를 수정한다 (등록 정보 수정 예시 : { 2호선, [강남역, 신림역, 보라매역] }) (덮어씌우기)
- [ ] 노선으로 등록 정보를 삭제한다
- [ ] 등록 목록을 조회한다
